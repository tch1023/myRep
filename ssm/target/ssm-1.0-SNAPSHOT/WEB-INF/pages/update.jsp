        ...
        $(function () {
            $.ajax({
                ...
                success:function (data) {
                    $("#address").append("<option value='${user.address}'>${user.address}</option>")
                    for (var i=0;i<data.length;i++){
                        if(data[i].city.match("${user.address}")){
                            return;
                        }else {
...
<div class="container">
    ...
    <form action="${pageContext.request.contextPath}/user/updateUser" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" value="${user.username}" id="name" name="username" placeholder="请输入姓名">
        </div>
        <div class="form-group">
            <label for="pwd">密码：</label>
            <input type="password" class="form-control" value="${user.pwd}" id="pwd" name="pwd" placeholder="请输入密码">
        </div>
        <div class="form-group">
